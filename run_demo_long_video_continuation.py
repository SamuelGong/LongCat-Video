...
def generate(args):
    ...
    # Time model loading (already done above)
    model_loading_time = time.time() - total_start_time
    timing_info["model_loading_time"] = model_loading_time
    
    # Time video loading
    video_loading_start = time.time()
    # (video loading already done above)
    video_loading_time = time.time() - video_loading_start
    timing_info["video_loading_time"] = video_loading_time
    ...
    if local_rank == 0:
        ...
        timing_file = output_path.replace('.mp4', '_timing.json')
